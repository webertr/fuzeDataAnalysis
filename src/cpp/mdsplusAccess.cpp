...
namespace MDSplusAccess {
  ...
  gsl_matrix_ushort *readMDSplusMatrix(int shotNumber, std::string nodeName,
    ...
    int numDim;
    int *shape = NULL;
    unsigned short *ushortArray = NULL;
  ...
  double readMDSplusDouble(int shotNumber, std::string nodeName,
			   std::string treeName) {
  ...
  int writeMDSplusMatrix(gsl_matrix_ushort *matIn, int shotNumber,
    ...
    MDSplus::Tree *tree = NULL;
    MDSplus::TreeNode *node = NULL;
    MDSplus::Uint16Array *data = NULL;
    unsigned short *dataShort = NULL;
    std::string readOnly = "NORMAL";
     ...
     */
    dataShort = (unsigned short *)malloc(numRows*numCols * sizeof(short));
    for (ii = 0; ii < numRows; ii++) {
      for (jj = 0; jj < numCols; jj++) {
    ...
    data = new MDSplus::Uint16Array((unsigned short *const)dataShort, 2, dims);
  ...
  bool readMDSplusVectorDimWrite(int shotNumber, std::string nodeName,
				 std::string treeName, std::string fileName) {
  ...
  static bool testMDSplusSignalWrite();

  int testMDSplusAccess() {

    if (!testMDSplusSignalWrite()) {
      std::cout << "Test MDSplus Signal Write test Failed\n";
      return -1;
    }
  ...
  static int testMDSplusWriteDouble() {
    ...
    if ( (abs(ret - val) > 1E-8) ) {
      std::cout << "Test failed. Value should be " << val << " but read back: "
		<< ret << "\n";
      return 0;
    }
  ...
  static bool testMDSplusSignalWrite() {
    ...
    int shotNumber = 201209002;
    std::string nodeName = "\\TOP.SIGNALS.XRAY:XRAY_5:RAW";
    std::string treeName = "fuze";
    gsl_vector *vecX = gsl_vector_alloc(5);
    gsl_vector *vecY = gsl_vector_alloc(5);
    for (int ii = 0; ii < 5; ii++){
      gsl_vector_set(vecX, ii, 1.0*ii);
      gsl_vector_set(vecY, ii, 1.0*ii*ii);
    }
    writeMDSplusSignal(vecX, vecY, shotNumber, nodeName, treeName);

    gsl_vector *vecYRead = readMDSplusVector(shotNumber, nodeName, treeName);
    gsl_vector *vecXRead = readMDSplusVectorDim(shotNumber, nodeName, treeName);
    double x, xRead, y, yRead;
    for (int ii = 0; ii < 5; ii++){
      x = (int) gsl_vector_get(vecX, ii);
      xRead = (int) gsl_vector_get(vecXRead, ii);
      y = (int) gsl_vector_get(vecY, ii);
      yRead = (int) gsl_vector_get(vecYRead, ii);
      if (x != xRead) {
	std::cout << "Error in testing MDSPlus Signal write. Readback does not"
		  << " match write. Expected " << x << "but got " << xRead;
	return false;
      }
      if (y != yRead) {
	std::cout << "Error in testing MDSPlus Signal write. Readback does not"
		  << " match write. Expected " << y << "but got " << yRead;
	return false;
      }
    }
    ...
    gsl_vector_free(vecXRead);
    gsl_vector_free(vecYRead);
