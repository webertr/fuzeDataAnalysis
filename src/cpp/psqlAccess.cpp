...
/******************************************************************************
 * Function: getShotNumbersBeforePulse
 * Inputs: int
 * Returns: gsl_vector *
 * Description: Get the most recent shot numbers prior to the passed number
 * and return them as a gsl vector long in descending order
 ******************************************************************************/
...
gsl_vector_long *getShotNumbersBeforePulse(int shotNums, long shotNumber) {
  ...
  try {
    ...
    /* Create SQL statement */
    oss << "SELECT shotnumber FROM shots WHERE shotnumber<=" << shotNumber
	<< "ORDER BY shotnumber DESC LIMIT " << shotNums;
    std::string sql = oss.str();
